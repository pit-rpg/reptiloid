...
def dampImg(model: Net, index: int, name: str, dataset: datasets.ImageFolder, device):
    p = Path(f'tmp/facesAutoencoder-pure-simple-sobel/{index}')
    if not p.exists():
        p.mkdir(parents=True, exist_ok=True)
        img = dataset[index][0]
        img = inv_normalize(img)
        save_image(img, f'tmp/facesAutoencoder-pure-simple-sobel/{index}/000.png')

    img = dataset[index][0].to(device).view(1, 3, 218, 178)
    img = inv_normalize(img)
    out = model(img).to('cpu').view(3, 218, 178)
    save_image(out, f'tmp/facesAutoencoder-pure-simple-sobel/{index}/{name}_{frame_number}.png')
...
def main():
    ...
    state = State('save', 'FACES_conv_autoencoder-pure-simple-sobel', model, optimizer, args)
    state.load_net()
