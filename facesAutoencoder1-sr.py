...
def dampImg(model: Net, index: int, name: str, dataset: datasets.ImageFolder, device):
    p = Path(f'tmp/facesAutoencoder1-srr/{index}')
    if not p.exists():
        p.mkdir(parents=True, exist_ok=True)
        img = dataset[index][0]
        save_image(img, f'tmp/facesAutoencoder1-srr/{index}/000.png')

    img = dataset[index][0].to(device).view(1, 3, 218, 178)
    out = model(img).to('cpu').view(3, 218, 178)
    save_image(out, f'tmp/facesAutoencoder1-srr/{index}/{name}_{frame_number}.png')
